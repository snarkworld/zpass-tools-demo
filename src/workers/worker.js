  ...
  if (ev.data.type === 'ALEO_EXECUTE_PROGRAM_LOCAL') {
    ...
    (async function () {
      try {
        ...
        console.log(
          `Web worker: Local execution completed in ${performance.now() - startTime} ms`
        );
    ...
    (async function () {
      try {
        ...
        // Ensure the program is valid and that it contains the function specified
        const program =
          await programManager.networkClient.getProgramObject(remoteProgram);
        const program_id = program.id();
        if (!program.hasFunction(aleoFunction)) {
          throw `Program ${program_id} does not contain function ${aleoFunction}`;
        }
        ...
        if (!programManager.keyProvider.containsKeys(cacheKey)) {
          console.log(
            `Web worker: Synthesizing proving & verifying keys for: '${program_id}:${aleoFunction}'`
          );
          const keys = await programManager.synthesizeKeys(
            remoteProgram,
            aleoFunction,
        ...
        console.log(
          `Web worker: On-chain execution transaction created in ${performance.now() - startTime} ms`
        );
        self.postMessage({
    ...
    const { remoteProgram, privateKey, aleoFunction, inputs, url } = ev.data;
    ...
    (async function () {
      try {
        ...
        const cacheKey = `${program_id}:${aleoFunction}`;
        const imports =
          await programManager.networkClient.getProgramImports(remoteProgram);
        ...
        const [provingKey, verifyingKey] =
          programManager.keyProvider.getKeys(cacheKey);
        ...
        console.log(
          `Web worker: Execution fee estimated in ${performance.now() - startTime} ms`
        );
    ...
    (async function () {
      try {
        const imports =
          await programManager.networkClient.getProgramImports(program);
        console.log('Estimating deployment fee..');
        let deploymentFee = await aleo.ProgramManagerBase.estimateDeploymentFee(
          program,
          imports
        );
        ...
        console.log(
          `Web worker: Deployment fee estimation completed in ${performance.now() - startTime} ms`
        );
    ...
    (async function () {
      try {
        ...
        // Return the transaction id to the main thread
        console.log(
          `Web worker: Transfer transaction ${transaction} created in ${performance.now() - startTime} ms`
        );
        self.postMessage({
    ...
    (async function () {
      try {
        ...
        // Return the transaction id to the main thread
        console.log(
          `Web worker: Deployment transaction ${transaction} created in ${performance.now() - startTime} ms`
        );
        self.postMessage({
    ...
    (async function () {
      try {
        ...
        // Return the transaction id to the main thread
        console.log(
          `Web worker: Split transaction ${transaction} created in ${performance.now() - startTime} ms`
        );
        self.postMessage({
    ...
    (async function () {
      try {
        ...
        // Return the transaction id to the main thread
        console.log(
          `Web worker: Join transaction ${transaction} created in ${performance.now() - startTime} ms`
        );
        self.postMessage({
